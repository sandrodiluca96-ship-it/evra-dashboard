...
with tabs[1]:
    ...
    if reps:
        ...
        if selected == "Estrazione" and len(sem_master):
            ...
            if "Droga_Codice" in my.columns and len(my):
                ...
                fig = px.bar(
                    ...
                    custom_data=["Droga_Descrizione","Droga_Kg","Kg_Semilav","N_Semilav","N_Lotti"],
                    text_auto=".1%"
                )
                fig.update_traces(hovertemplate="<b>%{y}</b><br>Descrizione droga: %{customdata[0]}<br>Mass Yield: %{x:.1%}<br>Kg droga: %{customdata[1]:,.2f}<br>Kg semilav: %{customdata[2]:,.2f}<br>N° semilav: %{customdata[3]}<br>N° lotti: %{customdata[4]}<extra></extra>")
