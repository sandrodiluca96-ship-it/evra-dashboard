...
with tabs[1]:
    ...
    if reps:
        ...
        if selected == "Estrazione" and len(sem_lotti):
            st.markdown("### Mass Yield e taglio malto per droga lavorata")
            my_lotti = sem_lotti.dropna(subset=["Mass_Yield"]).copy()
            if "Droga_Codice" in my_lotti.columns:
                my_lotti = my_lotti[my_lotti["Droga_Codice"].astype(str).str.len() > 0].copy()
                ...
                if droga_sel:
                    my_lotti = my_lotti[my_lotti["Droga_Codice"] == droga_sel].copy()

                my_lotti["Mass_Yield_%"] = my_lotti["Mass_Yield"] * 100
                my_lotti["Taglio_Malto_%"] = my_lotti["Taglio_Malto"] * 100

                yearly_drug = my_lotti.groupby(["Anno","Droga_Codice","Droga_Descrizione"], as_index=False).agg(
                ...
                if len(yearly_drug):
                    ...
                    with cmy1:
                        fig = px.line(
                            ...
                            markers=True,
                            title="Trend annuo Mass Yield medio",
                    ...
                    with cmy2:
                        fig = px.line(
                            ...
                            color="Droga_Codice" if not droga_sel else None,
                            markers=True,
                            title="Trend annuo taglio malto medio",
                            custom_data=["Droga_Descrizione","Droga_Kg","Kg_Semilav","N_Lotti"]
                        )
                ...
                st.markdown("### Elenco per droga lavorata")
                drug_table = my_lotti.groupby(["Droga_Codice","Droga_Descrizione"], as_index=False).agg(
                    Mass_Yield_Medio=("Mass_Yield_%","mean"),
                    Taglio_Malto_Medio=("Taglio_Malto_%","mean"),
                    Droga_Kg=("Droga_Kg","sum"),
                    Kg_Semilav=("Kg","sum"),
                ...
                st.markdown("### Dettaglio semilavorati collegati")
                sem_detail = my_lotti.copy()
                sem_detail["Mass_Yield_%"] = sem_detail["Mass_Yield"] * 100
                sem_detail["Taglio_Malto_%"] = sem_detail["Taglio_Malto"] * 100
                detail_cols = ["Droga_Codice","Droga_Descrizione","Codice","Descrizione","Lotto","Data","Kg","Droga_Kg","Mass_Yield_%","Taglio_Malto_%"]
                detail_cols = [c for c in detail_cols if c in sem_detail.columns]
                st.dataframe(
                    sem_detail[detail_cols].sort_values("Data", ascending=False),
                    use_container_width=True,
                    height=360,
                    column_config={
