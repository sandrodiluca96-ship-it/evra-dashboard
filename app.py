...
with tabs[1]:
    ...
    if reps:
        ...
        if selected == "Estrazione" and len(sem_master):
            st.markdown("### Mass Yield per droga lavorata")
            my = sem_master.dropna(subset=["Mass_Yield"]).copy()

            if "Droga_Codice" in my.columns:
                my = my[my["Droga_Codice"].astype(str).str.len() > 0].copy()

                my_drug = my.groupby(["Droga_Codice","Droga_Descrizione"], as_index=False).agg(
                    Mass_Yield=("Mass_Yield","mean"),
                    Taglio_Malto=("Taglio_Malto","mean"),
                    Droga_Kg=("Droga_Kg","sum"),
                    Kg_Semilav=("Kg","sum"),
                    N_Semilav=("Codice","nunique"),
                    N_Lotti=("N_Lotti","sum")
                ).sort_values("Droga_Kg", ascending=False).head(25)

                if len(my_drug):
                    my_drug["Mass_Yield_%"] = my_drug["Mass_Yield"] * 100
                    my_drug["Taglio_Malto_%"] = my_drug["Taglio_Malto"] * 100

                    fig = px.bar(
                        my_drug.sort_values("Mass_Yield_%", ascending=True),
                        x="Mass_Yield_%",
                        y="Droga_Codice",
                        orientation="h",
                        title="Mass Yield medio per droga MDR lavorata",
                        custom_data=["Droga_Descrizione","Droga_Kg","Kg_Semilav","N_Semilav","N_Lotti","Taglio_Malto_%"],
                        text="Mass_Yield_%"
                    )
                    fig.update_traces(
                        texttemplate="%{text:.1f}%",
                        hovertemplate="<b>%{y}</b><br>Descrizione droga: %{customdata[0]}<br>Mass Yield: %{x:.1f}%<br>Taglio medio: %{customdata[5]:.1f}%<br>Kg droga: %{customdata[1]:,.2f}<br>Kg semilav: %{customdata[2]:,.2f}<br>N° semilav: %{customdata[3]}<br>N° lotti: %{customdata[4]}<extra></extra>"
                    )
                    fig.update_xaxes(ticksuffix="%", title="Mass Yield")
                    layout(fig, 520)
                    st.plotly_chart(fig, use_container_width=True)

            st.markdown("### Elenco semilavorati: Mass Yield e taglio medio")
            sem_table = sem_master.copy()
            if "Mass_Yield" in sem_table.columns:
                sem_table["Mass_Yield_%"] = sem_table["Mass_Yield"] * 100
            if "Taglio_Malto" in sem_table.columns:
                sem_table["Taglio_Malto_%"] = sem_table["Taglio_Malto"] * 100
            cols = ["Codice","Descrizione","Droga_Codice","Droga_Descrizione","Kg","Droga_Kg","N_Lotti","Mass_Yield_%","Taglio_Malto_%"]
            cols = [c for c in cols if c in sem_table.columns]
            sem_table = sem_table[cols].sort_values("Kg", ascending=False)
            st.dataframe(
                sem_table,
                ...
                column_config={
                    ...
                    "Taglio_Malto_%": st.column_config.NumberColumn("Taglio medio malto", format="%.1f%%"),
                    "Kg": st.column_config.NumberColumn("Kg semilav", format="%.0f"),
...
with tabs[4]:
    ...
    if filtered_acq.empty:
        ...
        annual_line(annual_acq, "Anno", metric_acq, f"Trend annuale acquisti - {metric_acq}", ["N_Articoli","N_Fornitori"], 420)
...
with tabs[5]:
    ...
    if filtered_ven.empty:
        ...
        annual_line(annual_ven, "Anno", metric_v, f"Trend annuale vendite - {metric_v}", ["N_Clienti","N_Codici"], 420)
        ...
        if codice_ven:
                ...
                met_art_ven = st.radio("Metrica articolo venduto", ["Fatturato","Kg","Prezzo_medio"], horizontal=True, key="met_art_ven", help="Fatturato, quantità o prezzo medio €/kg.")
                andamento = andamento.sort_values(["Anno","Mese_Num"]).copy()
                andamento["Anno_Label"] = andamento["Anno"].astype(int).astype(str)
