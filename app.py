...
def select_lotto_from_df(df, lotto_col, code_col, desc_col, label, key):
    ...
    selected_label = st.selectbox(
        ...
        key=key,
        help="Puoi digitare il lotto, il codice prodotto o parte della descrizione."
    )
    return selected_label.split(" | ")[0] if selected_label else None
...
with tabs[3]:
    ...
    st.caption("Seleziona un lotto/commessa per vedere tutti i materiali utilizzati e la relativa percentuale.")
    ...
    if len(lotto_base):
        ...
        if lotto_sel:
            ...
            if len(lotto_df):
                ...
                materiali = lotto_df.groupby(["COD_COMP","DES_COMP"], as_index=False).agg(
                    Kg_Materiale=("QTA_LOTTO","sum")
                )
                materiali["% su prodotto"] = np.where(qta_finito > 0, materiali["Kg_Materiale"] / qta_finito * 100, np.nan)
                totale_materiali = materiali["Kg_Materiale"].sum()
                materiali["% su totale materiali"] = np.where(totale_materiali > 0, materiali["Kg_Materiale"] / totale_materiali * 100, np.nan)
                ...
                fig = px.bar(
                    ...
                    custom_data=["DES_COMP","Kg_Materiale","% su totale materiali","Tipo materiale"],
                    text="% su prodotto"
                )
                fig.update_traces(
                    texttemplate="%{text:.1f}%",
                    hovertemplate="<b>%{y}</b><br>Descrizione: %{customdata[0]}<br>Kg: %{customdata[1]:,.2f}<br>% su prodotto: %{x:.2f}%<br>% su totale materiali: %{customdata[2]:.2f}%<br>Tipo: %{customdata[3]}<extra></extra>"
                )
                ...
                st.dataframe(
                    materiali[["COD_COMP","DES_COMP","Tipo materiale","Kg_Materiale","% su prodotto","% su totale materiali"]],
                    ...
                    column_config={
                        ...
                        "% su totale materiali": st.column_config.NumberColumn("% su totale materiali", format="%.2f%%"),
                    }
                )
