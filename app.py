...
with tabs[1]:
    ...
    if reps:
        ...
        if selected == "Estrazione" and len(drug_yield):
            ...
            dy_all = drug_yield.dropna(subset=["Mass_Yield_%"]).copy()
            dy_all = dy_all[dy_all["Droga_Codice"].astype(str).str.len() > 0].copy()

            if len(dy_all):
                mass_yield_media = dy_all["Mass_Yield_%"].mean()
                taglio_medio = dy_all["Taglio_Malto_%"].mean()

                ky1, ky2 = st.columns(2)
                ky1.metric("Mass Yield media", f"{mass_yield_media:.1f}%")
                ...
                if droga_sel:
                    ...
                    d1.metric("Mass Yield media droga", f"{droga_data['Mass_Yield_%'].mean():.1f}%")
                    ...
                    if len(droga_ann):
                        fig = px.line(
                            ...
                            y="Mass_Yield_Medio",
                            markers=True,
                            title=f"Andamento Mass Yield - {droga_sel}",
                            custom_data=["Droga_Kg","Kg_Semilav","N_Lotti"]
                        )
                        fig.update_traces(hovertemplate="<b>%{x}</b><br>Mass Yield: %{y:.1f}%<br>Kg droga: %{customdata[0]:,.2f}<br>Kg semilav: %{customdata[1]:,.2f}<br>Lotti: %{customdata[2]}<extra></extra>")
                ...
                if sem_sel:
                    ...
                    s1.metric("Taglio medio malto", f"{sem_data['Taglio_Malto_%'].mean():.1f}%")
                    s2.metric("Mass Yield media", f"{sem_data['Mass_Yield_%'].mean():.1f}%")
                    ...
                    if len(sem_ann):
                        ...
                        fig.update_traces(hovertemplate="<b>%{x}</b><br>Taglio malto: %{y:.1f}%<br>Mass Yield: %{customdata[0]:.1f}%<br>Kg semilav: %{customdata[1]:,.2f}<br>Lotti: %{customdata[2]}<extra></extra>")
                        fig.update_yaxes(ticksuffix="%")
                        fig.update_xaxes(type="category")
                        layout(fig, 380)
                        st.plotly_chart(fig, use_container_width=True)
