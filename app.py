...
with tabs[5]:
    ...
    if filtered_acq.empty:
        ...
        st.markdown("### Trend acquisti globale")
        ...
        fig = px.line(
            month.sort_values(["Anno","Mese_Num"]),
            x="Mese_Nome",
            y=metric_acq,
            color="Anno",
            markers=True,
            category_orders={"Mese_Nome":MONTH_ORDER},
            title=f"Trend mensile acquisti - {metric_acq}",
...
with tabs[6]:
    ...
    if filtered_ven.empty:
        ...
        st.markdown("### Trend vendite globale")
        ...
        fig = px.line(
            ...
            markers=True,
            category_orders={"Mese_Nome":MONTH_ORDER},
            title=f"Trend mensile vendite - {metric_v}",
...
with tabs[7]:
    ...
    with c1:
        ...
        if not filtered_ven.empty:
            df = filtered_ven.groupby("RAGSOC", as_index=False).agg(Fatturato=("IMPORTO","sum"), Kg=("QTA","sum"), N_Codici=("CODART","nunique")).sort_values("Fatturato", ascending=False).head(10)
            st.dataframe(df, use_container_width=True)

        st.markdown("### Trend per cliente")
        if not filtered_ven.empty:
            ...
            met_c = st.radio("Metrica cliente", ["Fatturato","Kg"], horizontal=True)
            fig = px.line(cm.sort_values(["Anno","Mese_Num"]), x="Mese_Nome", y=met_c, color="Anno", markers=True, category_orders={"Mese_Nome":MONTH_ORDER}, title=f"Trend cliente - {cliente}", custom_data=["Anno","N_Codici"])
            fig.update_traces(hovertemplate="<b>%{x} %{customdata[0]}</b><br>Valore: %{y:,.2f}<br>Codici: %{customdata[1]}<extra></extra>")
            layout(fig, 420)
            st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Top fornitori")
        if not filtered_acq.empty:
            df = filtered_acq.groupby("ANDESCRI", as_index=False).agg(Valore=("TOTVEN","sum"), Kg=("QTAVEN","sum"), N_Articoli=("MVCODART","nunique")).sort_values("Valore", ascending=False).head(10)
            st.dataframe(df, use_container_width=True)

        st.markdown("### Trend per fornitore")
        if not filtered_acq.empty:
            fornitori = sorted(filtered_acq["ANDESCRI"].dropna().unique())
            fornitore = st.selectbox("Seleziona fornitore", fornitori)
            ...
            met_f = st.radio("Metrica fornitore", ["Valore","Kg"], horizontal=True)
            fig = px.line(fm.sort_values(["Anno","Mese_Num"]), x="Mese_Nome", y=met_f, color="Anno", markers=True, category_orders={"Mese_Nome":MONTH_ORDER}, title=f"Trend fornitore - {fornitore}", custom_data=["Anno","N_Articoli"])
            fig.update_traces(hovertemplate="<b>%{x} %{customdata[0]}</b><br>Valore: %{y:,.2f}<br>Articoli: %{customdata[1]}<extra></extra>")
            layout(fig, 420)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with c2:
        ...
        estr = filtered_rep_work[filtered_rep_work["Reparto"]=="Estrazione"]
        if len(estr):
