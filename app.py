...
def acquisto_categoria(code, desc="", gruppo=""):
    ...
    esclusioni = [
        "cespite", "ac-cespite", "macchina", "macchin", "servizio", "manutenz",
        "ricambio", "attrezz", "impianto", "consulenza", "trasporto", "noleggio",
        "materiale laboratorio", "dpi", "imballo", "confezion", "analisi"
    ]
    if code.startswith("AC") or any(x in txt for x in esclusioni):
        return "Escludi"
...
with tabs[0]:
    ...
    q_exec = st.text_input("Filtra codice o descrizione", key="exec_code_filter", placeholder="Scrivi codice o descrizione...")
    if q_exec:
        q = q_exec.lower()
        prod = filtered_lots[
            filtered_lots["CODART"].str.lower().str.contains(q, na=False) |
            filtered_lots["Descrizione"].str.lower().str.contains(q, na=False)
        ].copy()
        if len(prod):
            codici = sorted(prod["CODART"].dropna().unique())
            codice_sel = st.selectbox("Scegli codice", codici, key="exec_codice_select")
            prod_code = prod[prod["CODART"] == codice_sel]
...
with tabs[1]:
    ...
    if reps:
        ...
        if selected == "Estrazione" and len(sem_master):
            st.markdown("### Mass Yield semilavorati")
            my = sem_master.dropna(subset=["Mass_Yield"]).copy().sort_values("Kg", ascending=False).head(25)
            if len(my):
                fig = px.scatter(
                    my,
                    x="Kg",
                    ...
                    size="Kg",
                    hover_name="Codice",
                    custom_data=["Descrizione","N_Lotti","Taglio_Malto"],
                    title="Mass Yield semilavorati principali"
                )
                fig.update_traces(hovertemplate="<b>%{hovertext}</b><br>Descrizione: %{customdata[0]}<br>Kg: %{x:,.2f}<br>Lotti: %{customdata[1]}<br>Taglio malto: %{customdata[2]:.1%}<br>Mass Yield: %{y:.1%}<extra></extra>")
...
with tabs[2]:
    ...
    for tab, (title, fam) in zip(subtabs, defs):
        with tab:
            ...
            q_fam = st.text_input("Filtra codice o descrizione", key=f"family_filter_{fam}", placeholder="Scrivi codice o descrizione...")
            detail_data = data.copy()
            if q_fam:
                q = q_fam.lower()
                detail_data = detail_data[
                    detail_data["CODART"].str.lower().str.contains(q, na=False) |
                    detail_data["Descrizione"].str.lower().str.contains(q, na=False)
                ]
            if len(detail_data):
                codici_fam = sorted(detail_data["CODART"].dropna().unique())
                codice_fam = st.selectbox("Scegli codice", codici_fam, key=f"family_code_{fam}")
                code_data = detail_data[detail_data["CODART"] == codice_fam]
...
with tabs[3]:
    ...
    if len(pf_trend_no_fluid):
        ...
        fig = px.bar(pf_long, x="Anno", y="Kg", color="Tipo malto", title="Kg malto diretta vs malto da semilavorato - estratti secchi finiti", barmode="group")
        layout(fig, 430)
        st.plotly_chart(fig, use_container_width=True)

    st.markdown("### Ricerca formulazione per codice")
    q_form = st.text_input("Filtra codice o descrizione", key="form_code_filter", placeholder="Scrivi codice o descrizione...")
    form_view = pf_form_no_fluid.copy()
    if q_form:
        q = q_form.lower()
        form_view = form_view[
            form_view["Codice"].str.lower().str.contains(q, na=False) |
            form_view["Descrizione"].str.lower().str.contains(q, na=False)
        ]
    if len(form_view):
        codice_form = st.selectbox("Scegli codice", sorted(form_view["Codice"].dropna().unique()), key="form_code_select")
...
with tabs[4]:
    ...
    if filtered_acq.empty:
        ...
        q_acq_code = st.text_input("Filtra codice o descrizione articolo acquistato", key="acq_code_filter", placeholder="Scrivi codice o descrizione...")
        if q_acq_code:
            q = q_acq_code.lower()
            acq_code_view = filtered_acq[
                filtered_acq["MVCODART"].str.lower().str.contains(q, na=False) |
                filtered_acq["ARDESART"].str.lower().str.contains(q, na=False)
            ].copy()
            if len(acq_code_view):
                codice_acq = st.selectbox("Scegli articolo acquistato", sorted(acq_code_view["MVCODART"].dropna().unique()), key="acq_code_select")
                ...
                fig = px.line(andamento, x="Mese_Nome", y=f"{met_art_acq}_Cumulato", color="Anno", color_discrete_map=YEAR_COLORS, markers=True, category_orders={"Mese_Nome":MONTH_ORDER}, title=f"Cumulativo acquisti articolo - {codice_acq}", custom_data=["Anno", met_art_acq])
                fig.update_traces(hovertemplate="<b>%{x} %{customdata[0]}</b><br>Cumulato: %{y:,.2f}<br>Mese: %{customdata[1]:,.2f}<extra></extra>")
                layout(fig, 420)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Nessun articolo acquistato trovato.")
...
with tabs[5]:
    ...
    if filtered_ven.empty:
        ...
        q_ven_code = st.text_input("Filtra codice o descrizione articolo venduto", key="ven_code_filter", placeholder="Scrivi codice o descrizione...")
        if q_ven_code:
            q = q_ven_code.lower()
            ven_code_view = filtered_ven[
                filtered_ven["CODART"].str.lower().str.contains(q, na=False) |
                filtered_ven["DESART"].str.lower().str.contains(q, na=False)
            ].copy()
            if len(ven_code_view):
                codice_ven = st.selectbox("Scegli articolo venduto", sorted(ven_code_view["CODART"].dropna().unique()), key="ven_code_select")
                ...
                met_art_ven = st.radio("Metrica articolo venduto", ["Fatturato","Kg","Prezzo_medio"], horizontal=True, key="met_art_ven")
                andamento = andamento.sort_values(["Anno","Mese_Num"]).copy()
                ...
                fig = px.line(andamento, x="Mese_Nome", y=ycol, color="Anno", color_discrete_map=YEAR_COLORS, markers=True, category_orders={"Mese_Nome":MONTH_ORDER}, title=titolo, custom_data=["Anno", met_art_ven])
                fig.update_traces(hovertemplate="<b>%{x} %{customdata[0]}</b><br>Valore: %{y:,.2f}<br>Mese: %{customdata[1]:,.2f}<extra></extra>")
                layout(fig, 420)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info("Nessun articolo venduto trovato.")
