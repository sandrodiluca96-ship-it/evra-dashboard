    ...
    if reps:
        ...
        if selected == "Estrazione" and len(sem_master):
            ...
            if "Droga_Codice" in my.columns:
                ...
                if len(my_drug):
                    fig = px.bar(
                        my_drug.sort_values("Mass_Yield", ascending=True),
                        x="Mass_Yield",
                        y="Droga_Codice",
                        orientation="h",
                        title="Mass Yield medio per droga MDR lavorata",
                        custom_data=["Droga_Descrizione","Droga_Kg","Kg_Semilav","N_Semilav","N_Lotti","Taglio_Malto"],
                        text_auto=".1%"
                    )
                    fig.update_traces(
                        hovertemplate="<b>%{y}</b><br>Descrizione droga: %{customdata[0]}<br>Mass Yield: %{x:.1%}<br>Taglio medio: %{customdata[5]:.1%}<br>Kg droga: %{customdata[1]:,.2f}<br>Kg semilav: %{customdata[2]:,.2f}<br>N° semilav: %{customdata[3]}<br>N° lotti: %{customdata[4]}<extra></extra>"
                    )
            ...
            cols = ["Codice","Descrizione","Droga_Codice","Droga_Descrizione","Kg","Droga_Kg","N_Lotti","Mass_Yield","Taglio_Malto"]
            ...
            st.dataframe(
                ...
                height=420,
                column_config={
                    "Mass_Yield": st.column_config.NumberColumn("Mass Yield", format="%.1f%%"),
                    "Taglio_Malto": st.column_config.NumberColumn("Taglio medio malto", format="%.1f%%"),
