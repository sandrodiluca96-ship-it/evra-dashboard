...
.green { border:1px solid #2f8f6b; }
.blue { border:1px solid #355f9c; }
.orange { border:1px solid #9a6a2f; }
.magenta { border:1px solid #8a4b73; }
.cyan { border:1px solid #3f7f8c; }
.purple { border:1px solid #6c568f; }
...
def build_production(comm, rep):
    ...
    for (code, lotto), g in detail.groupby(["CODART","LOTTO_FINITO"]):
        fam = famiglia(code)
        if fam not in ["Estratto secco finito","Conto lavoro","Fluido"]:
            continue
        data = g["Data"].max()
        kg = g["Kg_PF"].iloc[0]
        desc = g["Descrizione_PF"].iloc[0]
...
qta_droga = rep_work[rep_work["Reparto"]=="Estrazione"]["Kg_Lavorato"].sum()

k1,k2,k3,k4 = st.columns(4)
with k1: kpi_card("Qtà finiti", fmt(qta_finiti), "Estratti secchi finiti + conto lavoro", "green")
with k2: kpi_card("Qtà semilav", fmt(qta_semilav), "Semilavorati W/Y prodotti", "blue")
with k3: kpi_card("Qtà fluidi", fmt(qta_fluidi), "Prodotti fluidi", "cyan")
with k4: kpi_card("Qtà droga estratta", fmt(qta_droga), "Scarichi MDR da commesse", "magenta")

if len(ven) or len(acq):
    k5,k6,k7,k8 = st.columns(4)
    with k5: kpi_card("Kg venduti", fmt(ven["QTA"].sum()) if len(ven) else "0", "Vendite totali", "orange")
    with k6: kpi_card("Fatturato", money(ven["IMPORTO"].sum()) if len(ven) else "€ 0", "Importo venduto", "orange")
    with k7: kpi_card("Kg acquistati", fmt(acq["QTAVEN"].sum()) if len(acq) else "0", "Solo materie prime ed estratti", "purple")
    with k8: kpi_card("Valore acquisti", money(acq["TOTVEN"].sum()) if len(acq) else "€ 0", "Solo categorie tecniche", "purple")
...
with tabs[5]:
    st.header("Acquisti")
    if acq.empty:
        st.warning("File acquisti assente o senza categorie tecniche filtrate.")
    else:
        c1,c2,c3,c4 = st.columns(4)
        c1.metric("Kg acquistati", fmt(acq["QTAVEN"].sum()))
        c2.metric("Valore acquisti", money(acq["TOTVEN"].sum()))
        c3.metric("N° fornitori", fmt(acq["ANDESCRI"].nunique()))
        c4.metric("N° articoli", fmt(acq["MVCODART"].nunique()))

        l,r = st.columns(2)
        with l:
            cat = acq.groupby("Categoria", as_index=False).agg(Kg=("QTAVEN","sum"), Valore=("TOTVEN","sum")).sort_values("Kg", ascending=True)
            ...
            st.plotly_chart(fig, use_container_width=True)
        with r:
            top_for = acq.groupby("ANDESCRI", as_index=False).agg(Kg=("QTAVEN","sum"), Valore=("TOTVEN","sum"), N_Articoli=("MVCODART","nunique")).sort_values("Valore", ascending=False).head(15)
            top_bar(top_for, "Valore", "ANDESCRI", "Top fornitori per valore", ["Kg","N_Articoli"], 420)

        st.markdown("### Trend acquisti per categoria")
        month = acq.groupby(["Categoria","Anno","Mese_Num","Mese_Nome"], as_index=False).agg(Kg=("QTAVEN","sum"), Valore=("TOTVEN","sum"), N_Articoli=("MVCODART","nunique"))
        cats = sorted(month["Categoria"].dropna().unique())
        sel = st.multiselect("Categorie acquisti", cats, default=cats)
        m = month[month["Categoria"].isin(sel)]
        fig = px.line(m.sort_values(["Anno","Mese_Num"]), x="Mese_Nome", y="Kg", color="Anno", line_dash="Categoria", markers=True, category_orders={"Mese_Nome":MONTH_ORDER}, title="Trend mensile acquisti")
        layout(fig, 450)
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Top articoli acquistati")
        top_art = acq.groupby(["MVCODART","ARDESART","Categoria"], as_index=False).agg(Kg=("QTAVEN","sum"), Valore=("TOTVEN","sum"), N_Fornitori=("ANDESCRI","nunique")).sort_values("Valore", ascending=False).head(30)
...
with tabs[6]:
    st.header("Vendite")
    if ven.empty:
        st.warning("File vendite assente.")
    else:
        c1,c2,c3,c4 = st.columns(4)
        c1.metric("Kg venduti", fmt(ven["QTA"].sum()))
        c2.metric("Fatturato", money(ven["IMPORTO"].sum()))
        c3.metric("N° clienti", fmt(ven["RAGSOC"].nunique()))
        prezzo = ven["IMPORTO"].sum()/ven["QTA"].sum() if ven["QTA"].sum() else 0
        c4.metric("Prezzo medio €/kg", money(prezzo).replace("€ ",""))

        l,r = st.columns(2)
        with l:
            top_cli = ven.groupby("RAGSOC", as_index=False).agg(Kg=("QTA","sum"), Fatturato=("IMPORTO","sum"), N_Codici=("CODART","nunique")).sort_values("Fatturato", ascending=False).head(15)
            top_bar(top_cli, "Fatturato", "RAGSOC", "Top clienti per fatturato", ["Kg","N_Codici"], 430)
        with r:
            fam = ven.groupby("Famiglia", as_index=False).agg(Kg=("QTA","sum"), Fatturato=("IMPORTO","sum"), N_Codici=("CODART","nunique")).sort_values("Fatturato", ascending=True)
        ...
        st.markdown("### Trend vendite")
        month = ven.groupby(["Famiglia","Anno","Mese_Num","Mese_Nome"], as_index=False).agg(Kg=("QTA","sum"), Fatturato=("IMPORTO","sum"), N_Clienti=("RAGSOC","nunique"))
        metric_v = st.radio("Metrica vendite", ["Fatturato","Kg"], horizontal=True)
        fig = px.line(month.sort_values(["Anno","Mese_Num"]), x="Mese_Nome", y=metric_v, color="Anno", line_dash="Famiglia", markers=True, category_orders={"Mese_Nome":MONTH_ORDER}, title=f"Trend mensile vendite - {metric_v}")
        layout(fig, 480)
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Top articoli venduti")
        top_art = ven.groupby(["CODART","DESART","Famiglia"], as_index=False).agg(Kg=("QTA","sum"), Fatturato=("IMPORTO","sum"), N_Clienti=("RAGSOC","nunique")).sort_values("Fatturato", ascending=False).head(30)
        st.dataframe(top_art, use_container_width=True, height=420)
...
with tabs[7]:
    ...
    with c1:
        ...
        if not ven.empty:
            df = ven.groupby("RAGSOC", as_index=False).agg(Fatturato=("IMPORTO","sum"), Kg=("QTA","sum"), N_Codici=("CODART","nunique")).sort_values("Fatturato", ascending=False).head(10)
            st.dataframe(df, use_container_width=True)

        st.markdown("### Top fornitori")
        if not acq.empty:
            df = acq.groupby("ANDESCRI", as_index=False).agg(Valore=("TOTVEN","sum"), Kg=("QTAVEN","sum"), N_Articoli=("MVCODART","nunique")).sort_values("Valore", ascending=False).head(10)
            st.dataframe(df, use_container_width=True)

    with c2:
        st.markdown("### Top prodotti lavorati")
        df = filtered_lots.groupby(["CODART","Descrizione","Famiglia"], as_index=False).agg(Kg=("Kg","sum"), N_Lotti=("LOTTO_FINITO","nunique")).sort_values("Kg", ascending=False).head(10)
        st.dataframe(df, use_container_width=True)

        st.markdown("### Top droghe estratte")
        estr = rep_work[rep_work["Reparto"]=="Estrazione"]
        if len(estr):
            df = estr.groupby(["Codice","Descrizione"], as_index=False).agg(Kg=("Kg_Lavorato","sum"), N_Commesse=("Commessa","nunique")).sort_values("Kg", ascending=False).head(10)
            st.dataframe(df, use_container_width=True)
