...
def select_code_from_df(df, code_col, desc_col, label, key):
    ...
    search_txt = st.text_input(
        ...
        help="Il testo digitato resta visibile; sotto trovi solo i risultati filtrati."
    )
    ...
    if search_txt:
        ...
        opts_df = opts_df[
            opts_df[code_col].str.lower().str.contains(q, na=False) |
            opts_df[desc_col].str.lower().str.contains(q, na=False) |
            opts_df["_label"].str.lower().str.contains(q, na=False)
        ]

    opts = sorted(opts_df["_label"].dropna().unique())

    if search_txt and not opts:
    ...
    selected_label = st.selectbox(
        "Seleziona risultato",
        opts,
        index=0 if search_txt and len(opts) == 1 else None,
...
def select_lotto_from_df(df, lotto_col, code_col, desc_col, label, key):
    ...
    if search_txt:
        q = search_txt.lower().strip()
        opts_df = opts_df[
            opts_df[lotto_col].str.lower().str.contains(q, na=False) |
            opts_df[code_col].str.lower().str.contains(q, na=False) |
            opts_df[desc_col].str.lower().str.contains(q, na=False) |
            opts_df["_label"].str.lower().str.contains(q, na=False)
        ]
