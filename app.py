...
def make_label(code, desc, max_desc=58):
    code = str(code)
    desc = str(desc).strip()
...
def add_month_fields(df, date_col="Data"):
    df = df.copy()
    df["Anno"] = df[date_col].dt.year
    df["Mese_Num"] = df[date_col].dt.month
...
def build_model(comm, rep):
    ...
    lots = add_month_fields(lots, "Data")
    ...
    lots["Articolo_Label"] = lots.apply(lambda r: make_label(r["CODART"], r["Descrizione"]), axis=1)
    ...
    detail["Componente_Label"] = detail.apply(lambda r: make_label(r["COD_COMP"], r["Descrizione_Componente"]), axis=1)
    ...
    for (code, lotto), g in sem_src.groupby(["CODART", "LOTTO_FINITO"]):
        ...
        sem_records.append({
            ...
            "Articolo_Label": make_label(code, desc),
        })
    ...
    if len(sem_lotti):
        sem_master = sem_lotti.groupby("Codice", as_index=False).agg(
            ...
            DER=("DER", "mean"),
            Articolo_Label=("Articolo_Label", "first"),
        )
    else:
        sem_master = pd.DataFrame(columns=["Codice", "Descrizione", "Kg", "N_Lotti", "Taglio_Malto", "Mass_Yield", "DER", "Articolo_Label"])
    ...
    if rep is not None and len(rep):
        ...
        desc_art_col = next((c for c in rep.columns if c == "Descrizione"), None)
        ...
        if art_col and desc_lav_col:
            ...
            rep_tmp["Descrizione"] = rep_tmp[desc_art_col].astype(str).str.strip() if desc_art_col else ""
            ...
            reparto_rows.append(rep_tmp[["Reparto", "Codice", "Descrizione", "Commessa", "Data", "Kg_Lavorato"]])

    # Estrazione da MDR nelle commesse
    ...
    if reparto_rows:
        ...
        rep_work = add_month_fields(rep_work, "Data")
        rep_work["Articolo_Label"] = rep_work.apply(lambda r: make_label(r["Codice"], r["Descrizione"]), axis=1)
        ...
        rep_work = pd.DataFrame(columns=["Reparto", "Codice", "Descrizione", "Commessa", "Data", "Kg_Lavorato", "Anno", "Mese_Num", "Mese_Nome", "Articolo_Label"])
    ...
    return lots, detail, sem_master, sem_lotti, pf_form, rep_work, rep_summary, rep_year, rep_month_year


def plot_monthly_by_year(df, title, y_col="Kg_Lavorato"):
    if df.empty:
    ...
    fig = px.line(
        ...
        color="Anno",
        markers=True,
        category_orders={"Mese_Nome": list(MONTH_MAP.values())},
        title=title,
    )
...
def reparto_section(reparto, rep_work, rep_year, rep_month_year):
    st.subheader(reparto)

    rw = rep_work[rep_work["Reparto"] == reparto].copy()
    ry = rep_year[rep_year["Reparto"] == reparto].copy()
    rmy = rep_month_year[rep_month_year["Reparto"] == reparto].copy()
    ...
    with right:
        top = rw.groupby(["Codice", "Descrizione", "Articolo_Label"], as_index=False).agg(
            Kg_Lavorato=("Kg_Lavorato", "sum"),
            N_Commesse=("Commessa", "nunique"),
        ...
        fig = px.bar(
            ...
            y="Articolo_Label",
            orientation="h",
            hover_data=["Codice", "Descrizione", "N_Commesse"],
            title=f"{reparto} - top codici",
            text_auto=".2s",
        )
        fig.update_layout(template="plotly_dark", height=390, yaxis_title="")
        st.plotly_chart(fig, use_container_width=True)

    plot_monthly_by_year(rmy, f"{reparto} - trend mensile per anno")
...
def family_section(title, family_name, lots, pf_form=None):
    ...
    data_month = data.groupby(["Anno", "Mese_Num", "Mese_Nome"], as_index=False).agg(Kg=("Kg", "sum")).sort_values(["Anno", "Mese_Num"])
    ...
    with left:
        fig = px.line(
            ...
            markers=True,
            category_orders={"Mese_Nome": list(MONTH_MAP.values())},
            title=f"{title} - trend mensile per anno",
        )
        fig.update_layout(template="plotly_dark", height=390)
        st.plotly_chart(fig, use_container_width=True)

    with right:
        top = data.groupby(["CODART", "Descrizione", "Articolo_Label"], as_index=False)["Kg"].sum().sort_values("Kg", ascending=False).head(15)
        fig = px.bar(
            top.sort_values("Kg", ascending=True),
            x="Kg",
            y="Articolo_Label",
            orientation="h",
            hover_data=["CODART", "Descrizione"],
            title=f"{title} - top codici",
            text_auto=".2s",
        )
        fig.update_layout(template="plotly_dark", height=390, yaxis_title="")
        st.plotly_chart(fig, use_container_width=True)
    ...
    if family_name == "Estratto secco finito" and pf_form is not None and len(pf_form):
        ...
        with right:
            ...
            fig = px.bar(
                top_m.sort_values("Malto_Totale_%", ascending=True),
                x="Malto_Totale_%",
                y="Articolo_Label",
                orientation="h",
                hover_data=["Codice", "Descrizione"],
                title="Top codici per malto totale",
            )
            fig.update_layout(template="plotly_dark", height=380, yaxis_title="")
            fig.update_xaxes(tickformat=".0%")
            st.plotly_chart(fig, use_container_width=True)
...
lots, detail, sem_master, sem_lotti, pf_form, rep_work, rep_summary, rep_year, rep_month_year = build_model(comm, rep)
...
with tab_exec:
    ...
    top = filtered_lots.groupby(["CODART", "Descrizione", "Famiglia", "Articolo_Label"], as_index=False)["Kg"].sum().sort_values("Kg", ascending=False).head(20)
...
with tab_rep:
    ...
    if not reparti_finali:
        ...
        reparto_section(selected_reparto, rep_work, rep_year, rep_month_year)
...
with tab_fam:
    ...
    with fam_tabs[2]:
        ...
        with right:
            ...
            fig = px.bar(top_sem.sort_values("Kg", ascending=True), x="Kg", y="Articolo_Label", orientation="h", hover_data=["Codice", "Descrizione"], title="Top semilavorati")
...
with tab_form:
    ...
    with left:
        ...
        fig = px.bar(top_malto.sort_values("Malto_Totale_%", ascending=True), x="Malto_Totale_%", y="Articolo_Label", orientation="h", hover_data=["Codice", "Descrizione"], title="Top prodotti per malto totale stimato")
        fig.update_layout(template="plotly_dark", height=520, yaxis_title="")
        fig.update_xaxes(tickformat=".0%")
        st.plotly_chart(fig, use_container_width=True)
