...
def build_model(comm, rep):
    ...
    if rep is not None and len(rep):
        ...
        if art_col and desc_lav_col:
            rep_tmp = rep.copy()
            rep_tmp["Reparto"] = rep_tmp[desc_lav_col].astype(str).str.strip()
            rep_tmp["Codice"] = rep_tmp[art_col].astype(str).str.strip()

            if data_rep_col:
                rep_tmp["Data_Reparto"] = pd.to_datetime(rep_tmp[data_rep_col], errors="coerce")
                rep_tmp["Anno"] = rep_tmp["Data_Reparto"].dt.year
            else:
                rep_tmp["Anno"] = np.nan

            if qty_rep_col:
                rep_tmp["Kg_Reparto"] = pd.to_numeric(rep_tmp[qty_rep_col], errors="coerce").fillna(0)
            else:
                rep_tmp["Kg_Reparto"] = 1
            ...
            rep_summary = rep_tmp.groupby("Reparto", as_index=False).agg(
                Kg_Reparto=("Kg_Reparto", "sum"),
                N_Righe=("Codice", "size"),
                N_Articoli=("Codice", "nunique"),
                N_Commesse=("Commessa", "nunique"),
            ).sort_values("Kg_Reparto", ascending=False)

            rep_year = rep_tmp.dropna(subset=["Anno"]).groupby(["Reparto", "Anno"], as_index=False).agg(
                Kg_Reparto=("Kg_Reparto", "sum"),
                N_Righe=("Codice", "size"),
                N_Articoli=("Codice", "nunique"),
                N_Commesse=("Commessa", "nunique"),
            )
            rep_year["Anno"] = rep_year["Anno"].astype(int)

            rep_map = rep_tmp.groupby("Codice", as_index=False).agg(
                Reparti=("Reparto", lambda x: " | ".join(sorted(set([str(v) for v in x if str(v).strip()])))),
                N_Lavorazioni=("Reparto", "nunique"),
            )

    return lots, detail, sem_master, sem_lotti, pf_form, rep_summary, rep_map, rep_year
...
lots, detail, sem_master, sem_lotti, pf_form, rep_summary, rep_map, rep_year = build_model(comm, rep)
...
with tab3:
    ...
    if len(rep_summary):
        ...
        c1.metric("Kg caricati reparto", f"{rep_summary['Kg_Reparto'].sum():,.0f}".replace(",", "."))
        c2.metric("N° reparti", f"{rep_summary['Reparto'].nunique():,}".replace(",", "."))
        c3.metric("N° articoli/reparto", f"{rep_summary['N_Articoli'].sum():,}".replace(",", "."))
        c4.metric("N° commesse", f"{rep_summary['N_Commesse'].sum():,}".replace(",", "."))

        st.markdown("### Produzione complessiva per reparto")
        fig = px.bar(
            rep_summary.sort_values("Kg_Reparto", ascending=True),
            x="Kg_Reparto",
            y="Reparto",
            orientation="h",
            title="Kg caricati per reparto",
            text_auto=".2s",
        )
        fig.update_layout(template="plotly_dark", height=520, margin=dict(l=10, r=10, t=50, b=10))
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Andamento per anno")
        if len(rep_year):
            ...
            metric_choice = st.radio(
                "Metrica",
                ["Kg_Reparto", "N_Commesse", "N_Articoli"],
                horizontal=True,
                format_func=lambda x: {
                    "Kg_Reparto": "Kg caricati",
                    "N_Commesse": "N° commesse",
                    "N_Articoli": "N° articoli",
                }[x],
            )

            rep_plot = rep_year[rep_year["Reparto"].isin(selected_reparti)].copy()
            ...
            fig = px.bar(
                ...
                color="Reparto",
                barmode="group",
                title="Confronto reparti per anno",
                text_auto=".2s",
            )
            ...
            st.dataframe(
                rep_year.sort_values(["Reparto", "Anno"]),
                use_container_width=True,
                height=420,
            )

        st.markdown("### Tabella riepilogo reparti")
