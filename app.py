...
# RELEASE OEE LAURIA 3.10.0 — importazione storico gestionale
...
def parse_historical_sources(carico_upload,explosion_upload):
    ...
    atom=atom[atom["_date"].notna()&(atom["_qty"]>0)&atom["_lot"].ne("")].copy()
    rows=[]; taglio_reale=0
    ...
    for keys,group in explosion.groupby(group_cols,dropna=False,sort=False):
        ...
        if not article.upper().startswith("WSD") or not group["_is_mdr"].any() or not lot:
            continue
